...
import { fetchAdultBikes } from "@/hooks/myAPI";

const renderStatusOverlay = (status) => {
    if (status === 'reserved') {
        return (
            <View style={[styles.overlay]}>
                <Text style={{ color: 'white', textAlign: 'center', fontSize: 24, fontFamily: 'mplusb' }}>Reserved</Text>
            </View>
        );
    } else if (status === 'rented') {
        return (
            <View style={[styles.overlay]}>
                <Text style={{ color: 'white', textAlign: 'center', fontSize: 24, fontFamily: 'mplusb' }}>Rented</Text>
            </View>
        );
    }
...
export default function Adults() {
    ...
    const getBikes = async () => {
        try {
            const data = await fetchAdultBikes();
            setAllAdult(data);
        } catch (err) {
